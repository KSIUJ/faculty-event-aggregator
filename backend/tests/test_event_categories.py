...
def test_create_event_category_without_icon(client: TestClient):
    payload = {"title": "Meetup"}
    res = client.post("/event-categories", json=payload)
    assert res.status_code == 201
    data = res.json()
    assert data["title"] == "Meetup"
...
def test_patch_event_category(client: TestClient):
    created = client.post("/event-categories", json={"title": "Old Title", "icon_name": "old_icon"}).json()
    cat_id = created["id"]

    res = client.patch(f"/event-categories/{cat_id}", json={"title": "New Title"})
    assert res.status_code == 200
    data = res.json()
...
def test_patch_event_category_not_found(client: TestClient):
    res = client.patch("/event-categories/999", json={"title": "Updated"})
    assert res.status_code == 404


def test_delete_event_category(client: TestClient):
    created = client.post("/event-categories", json={"title": "To Delete"}).json()
    cat_id = created["id"]
